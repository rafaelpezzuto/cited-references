...
def enrich(data, line_number, format, ignore_previous_result, title2issnl, issn2titles, title_year_volume2issn, artifitial_title_year_volume2issn, issn2equations, use_fuzzy):
    ...
    if not ignore_previous_result and 'cited_issnl' in cit.__dict__:
        ...
        standardize_data(cit)
        cited_journal_titles_cleaned, cited_year_cleaned, cited_volume_cleaned = extract_essential_data(cit)

        _total_titles = len(cited_journal_titles_cleaned)

        # Caso haja título de periódico 
        if len(cited_journal_titles_cleaned) > 0:
            for _idx, cjt_cleaned in enumerate(cited_journal_titles_cleaned):
                # Caso ocorra correspondência exata entre título de periódico e base de correção
                if cjt_cleaned in title2issnl:
                    exact_match_issnls = title2issnl.get(cjt_cleaned, '').split('#')
                    ...
                    # Correspondência com mais de um ISSN-L
                        ...
                        if cited_year_cleaned.isdigit():
                            ...
                            if cited_volume_cleaned != '':
                                ...
                                title_year_volume_key = '-'.join([cjt_cleaned, cited_year_cleaned, cited_volume_cleaned])
                            ...
                            for i in exact_match_issnls:
                                ...
                                if cit_volume_inferred:
                                    for voli in [vi for vi in range(cit_volume_inferred - 1, cit_volume_inferred + 2) if vi > 0]:
                                        ...
                                        if len(result_issn_to_data['issns']) > 1 and is_last_item(_idx, _total_titles):
                            ...
                            if cited_volume_cleaned != '':
                                key_title_year_volume = '-'.join([cjt_cleaned, cited_year_cleaned, cited_volume_cleaned])
                                ...
                                if cited_volume_cleaned != '':
                                    ...
                                    for fz_title in get_titles(fuzzy_match_issnls, issn2titles):
                                        ...
                                        if len(fz_possible_issnls) > 0:
                                            ...
                                            if len(fz_result_issn_to_data['issns']) > 1 and is_last_item(_idx, _total_titles):
